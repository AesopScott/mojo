...
- Mojo and MAPS+Org are paired source repos for the role/agent operating system. When a build changes org bootstrap behavior, role/agent rules, skills/templates, validators, heartbeat installer/source automation, or MAPS+Org source links, update both `C:\Users\scott\Code\mojo` and `C:\Users\scott\Code\maps+org` when a counterpart exists; if one side has no counterpart, record why.
...
- `/build-agent` v0.5.0 is the current Build contract for autonomy: Build must ask for and enforce an explicit Autonomy Contract Gate before implementing autonomous behavior, recurring triggers, scheduled checks, heartbeat actions, independent goal pursuit, self-continuation, or action outside a visible user-directed turn. Build may enforce or stub blocked autonomy, but it must not create missing autonomy authority.
...
- ASPA `/build-agent` A3 has a runtime-neutral local proof harness under `agents/vik-aspa`; `agents/vik-aspa/autonomy-contract.md` is the draft full bounded autonomy contract; 14 unittest checks pass for required contract loading, missing autonomy-contract fail-closed behavior, stricter profile limits, denied tool/memory/external/production/spending/autonomous/secrets/authority behavior, and runtime-specific target blocking. Runtime adapter proof and activation remain deferred until `/equip-agent`, `/evaluate-agent`, `/deploy-agent`, `/observe-agent`, rollback proof, and Scott approval.
...
- Treat Scott's autonomy clarification this way: automation is not autonomy, and ASPA should not stay non-autonomous forever. Build must ask for the autonomy contract when autonomy is requested or implied; ASPA now has a draft full bounded autonomy contract, but activation remains blocked until the contract is approved and equipped/evaluated/deployed/observed with rollback proof.

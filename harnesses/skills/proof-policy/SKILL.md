---
name: proof-policy
description: "Use when working on the Proof Authority harness and you need cost efficiency control: Scales required proof to risk tier and allows waiver shortcuts on low-risk paths to avoid over-proving cheap work."
---
...
- Command: /proof-policy

## What This Skill Does

Scales required proof to risk tier and allows waiver shortcuts on low-risk paths to avoid over-proving cheap work.

Changing this harness changes what evidence is enough to trust the work: which proofs are required, how strongly they are scored, when waivers are allowed, and what must be crosschecked before progress continues.
...
- You are changing or reviewing the Proof Authority harness.
...
1. Name the exact boundary: `Proof Authority` / `Cost efficiency`.
...
- Required-proof tiering by risk
- Waiver shortcuts for low-risk paths
...
2. Write the observed failure mode in one sentence: "Proof Policy is needed because ..."
...
- Positive test: a normal request uses `Proof Policy` and produces the expected controlled behavior.
- Boundary test: an overreach, missing-input, unsafe, expensive, or unsupported request is blocked, downgraded, or escalated.
- Regression check: existing Proof Authority behavior that should not change still works.
...
```markdown
## Proof Policy Harness Update

Boundary: Proof Authority / Cost efficiency

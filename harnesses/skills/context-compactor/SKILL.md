...
description: "Use when working on the Context harness and you need performance control: Compacts context and selects the right window size so prompts fit with less processing."
---
...
- Harness: Context
- Axis: Performance
- Command: /context-compactor

## What This Skill Does

Compacts context and selects the right window size so prompts fit with less processing.

Changing this harness changes what the model gets to know: context packing, trimming, summarization, chunk reuse, and per-model window rules that decide what fits into the request.
...
- You are changing or reviewing the Context harness.
...
1. Name the exact boundary: `Context` / `Performance`.
...
2. Write the observed failure mode in one sentence: "Context Compactor is needed because ..."
...
- Positive test: a normal request uses `Context Compactor` and produces the expected controlled behavior.
- Boundary test: an overreach, missing-input, unsafe, expensive, or unsupported request is blocked, downgraded, or escalated.
- Regression check: existing Context behavior that should not change still works.
...
```markdown
## Context Compactor Harness Update

Boundary: Context / Performance

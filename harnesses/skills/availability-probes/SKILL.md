...
description: "Use when working on the Latency harness and you need capability control: Adds SLO config and availability probes for new routes."
---
...
1. Name the exact boundary: `Latency` / `Capability`.
...
2. Write the observed failure mode in one sentence: "Availability Probes is needed because ..."
...
- Positive test: a normal request uses `Availability Probes` and produces the expected controlled behavior.
...
```markdown
## Availability Probes Harness Update

Boundary: Latency / Capability

---
name: goal-cache
description: "Use when working on the Goal harness and you need performance control: Caches the goal binding so each call reattaches to the objective without recompute."
---
...
Caches the goal binding so each call reattaches to the objective without recompute.
...
1. Name the exact boundary: `Goal` / `Performance`.
...
2. Write the observed failure mode in one sentence: "Goal Cache is needed because ..."
...
- Positive test: a normal request uses `Goal Cache` and produces the expected controlled behavior.
...
```markdown
## Goal Cache Harness Update

Boundary: Goal / Performance

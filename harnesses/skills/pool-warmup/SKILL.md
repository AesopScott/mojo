---
name: pool-warmup
description: "Use when working on the Browser Capture harness and you need performance control: Warms a headless browser pool and tunes capture resolution and diff sensitivity so page capture returns quickly."
---
...
1. Name the exact boundary: `Browser Capture` / `Performance`.
...
2. Write the observed failure mode in one sentence: "Pool Warmup is needed because ..."
...
- Positive test: a normal request uses `Pool Warmup` and produces the expected controlled behavior.
...
```markdown
## Pool Warmup Harness Update

Boundary: Browser Capture / Performance

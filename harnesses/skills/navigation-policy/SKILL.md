...
description: "Use when working on the Browser Capture harness and you need capability control: Adds navigation-allow policy, an action-planner strategy, and evidence-store connectors."
---
...
1. Name the exact boundary: `Browser Capture` / `Capability`.
...
- Navigation-allow policy
- Action-planner strategy
- Evidence-store connectors
...
2. Write the observed failure mode in one sentence: "Navigation Policy is needed because ..."
...
- Positive test: a normal request uses `Navigation Policy` and produces the expected controlled behavior.
...
```markdown
## Navigation Policy Harness Update

Boundary: Browser Capture / Capability

---
name: path-queries
description: "Use when working on the File Navigation harness and you need capability control: Adds ownership-rule sources, a capability-mapping schema, and custom capability-to-code-to-proof queries."
---
...
- Axis: Capability
- Command: /path-queries

## What This Skill Does

Adds ownership-rule sources, a capability-mapping schema, and custom capability-to-code-to-proof queries.
...
1. Name the exact boundary: `File Navigation` / `Capability`.
...
2. Write the observed failure mode in one sentence: "Path Queries is needed because ..."
...
- Positive test: a normal request uses `Path Queries` and produces the expected controlled behavior.
...
```markdown
## Path Queries Harness Update

Boundary: File Navigation / Capability

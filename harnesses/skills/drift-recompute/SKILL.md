...
description: "Use when working on the Project Scope harness and you need performance control: Recomputes only changed scope regions and caches the dependency graph so drift checks stay cheap and current."
---
...
- Axis: Performance
- Command: /drift-recompute

## What This Skill Does

Recomputes only changed scope regions and caches the dependency graph so drift checks stay cheap and current.
...
1. Name the exact boundary: `Project Scope` / `Performance`.
...
2. Write the observed failure mode in one sentence: "Drift Recompute is needed because ..."
...
- Positive test: a normal request uses `Drift Recompute` and produces the expected controlled behavior.
...
```markdown
## Drift Recompute Harness Update

Boundary: Project Scope / Performance

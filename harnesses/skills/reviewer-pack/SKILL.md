---
name: reviewer-pack
description: "Use when working on the Review Queue harness and you need capability control: Adds reviewer-agent sets, a configurable severity rubric, and a repair-verification gate."
---
...
Adds reviewer-agent sets, a configurable severity rubric, and a repair-verification gate.
...
1. Name the exact boundary: `Review Queue` / `Capability`.
...
- Severity rubric (config)
- Reviewer-agent set (plugin)
- Repair-verify gate
...
2. Write the observed failure mode in one sentence: "Reviewer Pack is needed because ..."
...
- Positive test: a normal request uses `Reviewer Pack` and produces the expected controlled behavior.
...
```markdown
## Reviewer Pack Harness Update

Boundary: Review Queue / Capability

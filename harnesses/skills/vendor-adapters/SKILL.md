...
description: "Use when working on the Provider harness and you need capability control: Adds per-vendor adapters and auth-mode config so new providers connect through the controlled boundary."
---
...
- Harness: Provider
- Axis: Capability
- Command: /vendor-adapters

## What This Skill Does

Adds per-vendor adapters and auth-mode config so new providers connect through the controlled boundary.

Changing this harness changes the vendor boundary for a call: which provider adapters, auth modes, endpoints, regions, and transport rules are available before the model is contacted.
...
- You are changing or reviewing the Provider harness.
...
1. Name the exact boundary: `Provider` / `Capability`.
...
2. Write the observed failure mode in one sentence: "Vendor Adapters is needed because ..."
...
- Positive test: a normal request uses `Vendor Adapters` and produces the expected controlled behavior.
- Boundary test: an overreach, missing-input, unsafe, expensive, or unsupported request is blocked, downgraded, or escalated.
- Regression check: existing Provider behavior that should not change still works.
...
```markdown
## Vendor Adapters Harness Update

Boundary: Provider / Capability

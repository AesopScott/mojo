...
description: "Use when working on the Goal harness and you need capability control: Adds objective-binding sources and a goal schema so new objective types bind cleanly."
---
...
1. Name the exact boundary: `Goal` / `Capability`.
...
2. Write the observed failure mode in one sentence: "Objective Binding is needed because ..."
...
- Positive test: a normal request uses `Objective Binding` and produces the expected controlled behavior.
...
```markdown
## Objective Binding Harness Update

Boundary: Goal / Capability

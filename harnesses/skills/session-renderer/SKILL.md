---
name: session-renderer
description: "Use when working on the Interface harness and you need performance control: Virtualizes large session lists and throttles refresh cadence so the command center stays responsive at high session counts."
---
...
- Harness: Interface
- Axis: Performance
- Command: /session-renderer

## What This Skill Does

Virtualizes large session lists and throttles refresh cadence so the command center stays responsive at high session counts.
...
1. Name the exact boundary: `Interface` / `Performance`.
...
- Render cadence / refresh throttle
...
2. Write the observed failure mode in one sentence: "Session Renderer is needed because ..."
...
- Positive test: a normal request uses `Session Renderer` and produces the expected controlled behavior.
...
```markdown
## Session Renderer Harness Update

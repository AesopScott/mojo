---
name: template-precompiler
description: "Use when working on the Prompt harness and you need performance control: Precompiles templates and reuses cache prefixes so prompt assembly is fast."
---
...
- Harness: Prompt
- Axis: Performance
- Command: /template-precompiler

## What This Skill Does

Precompiles templates and reuses cache prefixes so prompt assembly is fast.

Changing this harness changes the admitted instruction payload: templates, injection order, cache prefixes, and redundant-context removal that directly shape the model response.
...
- You are changing or reviewing the Prompt harness.
...
1. Name the exact boundary: `Prompt` / `Performance`.
...
2. Write the observed failure mode in one sentence: "Template Precompiler is needed because ..."
...
- Positive test: a normal request uses `Template Precompiler` and produces the expected controlled behavior.
- Boundary test: an overreach, missing-input, unsafe, expensive, or unsupported request is blocked, downgraded, or escalated.
- Regression check: existing Prompt behavior that should not change still works.
...
```markdown
## Template Precompiler Harness Update

Boundary: Prompt / Performance

---
name: scope-connectors
description: "Use when working on the Project Scope harness and you need capability control: Adds connectors and handoff templates so scope tracking spans new project and cross-project sources."
---
...
- Harness: Project Scope
- Axis: Capability
- Command: /scope-connectors

## What This Skill Does

Adds connectors and handoff templates so scope tracking spans new project and cross-project sources.

Changing this harness changes the map of what the system believes it is allowed to work on: objectives, dependencies, drift signals, and handoff boundaries that keep work inside the right project.
...
- You are changing or reviewing the Project Scope harness.
...
1. Name the exact boundary: `Project Scope` / `Capability`.
...
- Objective schema (config)
- Dependency-source connectors
- Handoff templates
...
2. Write the observed failure mode in one sentence: "Scope Connectors is needed because ..."
...
- Positive test: a normal request uses `Scope Connectors` and produces the expected controlled behavior.
- Boundary test: an overreach, missing-input, unsafe, expensive, or unsupported request is blocked, downgraded, or escalated.
- Regression check: existing Project Scope behavior that should not change still works.
...
```markdown
## Scope Connectors Harness Update

Boundary: Project Scope / Capability

...
description: "Use when working on the Identity harness and you need capability control: Maintains a version allowlist and identity-record schema so only approved model versions run."
---
...
Changing this harness changes reproducibility: the exact provider, model, and version that receive the request, which prevents silent model swaps from changing behavior without a trace.
...
- You are changing or reviewing the Identity harness.
...
1. Name the exact boundary: `Identity` / `Capability`.
...
2. Write the observed failure mode in one sentence: "Version Allowlist is needed because ..."
...
- Positive test: a normal request uses `Version Allowlist` and produces the expected controlled behavior.
- Boundary test: an overreach, missing-input, unsafe, expensive, or unsupported request is blocked, downgraded, or escalated.
- Regression check: existing Identity behavior that should not change still works.
...
```markdown
## Version Allowlist Harness Update

Boundary: Identity / Capability

---
name: chain-builder
description: "Use when working on the Fallback harness and you need capability control: Defines the fallback chain and retry policy for new failure modes."
---
...
Defines the fallback chain and retry policy for new failure modes.
...
1. Name the exact boundary: `Fallback` / `Capability`.
...
- Fallback chain definition (config)
- Retry policy
...
2. Write the observed failure mode in one sentence: "Chain Builder is needed because ..."
...
- Positive test: a normal request uses `Chain Builder` and produces the expected controlled behavior.
...
```markdown
## Chain Builder Harness Update

Boundary: Fallback / Capability

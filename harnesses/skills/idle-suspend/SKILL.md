---
name: idle-suspend
description: "Use when working on the Session Manager harness and you need cost efficiency control: Suspends idle sessions on a timer and compacts state with retention limits to shrink storage and live cost."
---
...
1. Name the exact boundary: `Session Manager` / `Cost efficiency`.
...
2. Write the observed failure mode in one sentence: "Idle Suspend is needed because ..."
...
- Positive test: a normal request uses `Idle Suspend` and produces the expected controlled behavior.
...
```markdown
## Idle Suspend Harness Update

Boundary: Session Manager / Cost efficiency

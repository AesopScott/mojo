---
name: handshake-cache
description: "Use when working on the Network Protocol harness and you need performance control: Caches trust handshakes and batches signature verification so multi-node calls add less per-packet latency."
---
...
- Harness: Network Protocol
- Axis: Performance
- Command: /handshake-cache

## What This Skill Does

Caches trust handshakes and batches signature verification so multi-node calls add less per-packet latency.

Changing this harness changes multi-node coordination: which remote systems are trusted, how capabilities are discovered, how packets are signed, and what degraded behavior is allowed when a node fails.
...
- You are changing or reviewing the Network Protocol harness.
...
1. Name the exact boundary: `Network Protocol` / `Performance`.
...
2. Write the observed failure mode in one sentence: "Handshake Cache is needed because ..."
...
- Positive test: a normal request uses `Handshake Cache` and produces the expected controlled behavior.
- Boundary test: an overreach, missing-input, unsafe, expensive, or unsupported request is blocked, downgraded, or escalated.
- Regression check: existing Network Protocol behavior that should not change still works.
...
```markdown
## Handshake Cache Harness Update

Boundary: Network Protocol / Performance

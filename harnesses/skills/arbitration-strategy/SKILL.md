...
description: "Use when working on the Orchestrator harness and you need capability control: Lets operators swap the \"why this next\" decision logic (round-robin, risk-first, deadline-first) without touching the orchestrator core."
---
...
1. Name the exact boundary: `Orchestrator` / `Capability`.
...
- Pluggable arbitration strategy (skill)
- Objective-source inputs (Project Scope / Goal feeds)
...
2. Write the observed failure mode in one sentence: "Arbitration Strategy is needed because ..."
...
- Positive test: a normal request uses `Arbitration Strategy` and produces the expected controlled behavior.
...
```markdown
## Arbitration Strategy Harness Update

Boundary: Orchestrator / Capability

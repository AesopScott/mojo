---
name: evidence-connectors
description: "Use when working on the Evidence harness and you need capability control: Adds connectors and a link schema so output ties to new validation sources."
---
...
- Harness: Evidence
- Axis: Capability
- Command: /evidence-connectors

## What This Skill Does

Adds connectors and a link schema so output ties to new validation sources.

Changing this harness changes validation linkage: which tests, citations, reviews, or proof records are attached to the model output before it is trusted.
...
- You are changing or reviewing the Evidence harness.
...
1. Name the exact boundary: `Evidence` / `Capability`.
...
- Evidence-source connectors (plugin)
- Link schema
...
2. Write the observed failure mode in one sentence: "Evidence Connectors is needed because ..."
...
- Positive test: a normal request uses `Evidence Connectors` and produces the expected controlled behavior.
- Boundary test: an overreach, missing-input, unsafe, expensive, or unsupported request is blocked, downgraded, or escalated.
- Regression check: existing Evidence behavior that should not change still works.
...
```markdown
## Evidence Connectors Harness Update

Boundary: Evidence / Capability

...
description: "Use when working on the Sub-agent Dispatch harness and you need performance control: Tunes lane concurrency and fan-out and prefers pipeline over barrier stages so sub-agent work overlaps instead of waiting."
---
...
- Axis: Performance
- Command: /stage-scheduler

## What This Skill Does

Tunes lane concurrency and fan-out and prefers pipeline over barrier stages so sub-agent work overlaps instead of waiting.

Changing this harness changes parallel execution: how many workers can run, what each worker receives, what result shape they must return, and how their output is merged back into the main session.
...
- You are changing or reviewing the Sub-agent Dispatch harness.
...
1. Name the exact boundary: `Sub-agent Dispatch` / `Performance`.
...
- Lane concurrency cap
- Fan-out width
- Pipeline vs. barrier stages
...
2. Write the observed failure mode in one sentence: "Stage Scheduler is needed because ..."
...
- Positive test: a normal request uses `Stage Scheduler` and produces the expected controlled behavior.
- Boundary test: an overreach, missing-input, unsafe, expensive, or unsupported request is blocked, downgraded, or escalated.
- Regression check: existing Sub-agent Dispatch behavior that should not change still works.
...
```markdown
## Stage Scheduler Harness Update

Boundary: Sub-agent Dispatch / Performance

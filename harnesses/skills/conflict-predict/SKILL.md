...
description: "Use when working on the Git Control harness and you need performance control: Precomputes likely merge conflicts and tunes shallow-fetch and diff-risk scoring so Git operations plan faster."
---
...
- Harness: Git Control
- Axis: Performance
- Command: /conflict-predict

## What This Skill Does

Precomputes likely merge conflicts and tunes shallow-fetch and diff-risk scoring so Git operations plan faster.

Changing this harness changes source-control authority: branch leases, diff-risk scoring, CI gates, merge behavior, and whether the system can write toward main without a human stop.
...
- You are changing or reviewing the Git Control harness.
...
1. Name the exact boundary: `Git Control` / `Performance`.
...
- Diff-risk model
- Conflict-predict precompute
- Shallow-fetch tuning
...
2. Write the observed failure mode in one sentence: "Conflict Predict is needed because ..."
...
- Positive test: a normal request uses `Conflict Predict` and produces the expected controlled behavior.
- Boundary test: an overreach, missing-input, unsafe, expensive, or unsupported request is blocked, downgraded, or escalated.
- Regression check: existing Git Control behavior that should not change still works.
...
```markdown
## Conflict Predict Harness Update

Boundary: Git Control / Performance

---
name: scaled-review
description: "Use when working on the Review Queue harness and you need cost efficiency control: Sets review depth and reviewer-model tier by severity and skips trivial findings to control review spend."
---
...
- Command: /scaled-review

## What This Skill Does

Sets review depth and reviewer-model tier by severity and skips trivial findings to control review spend.

Changing this harness changes the review pipeline: what severity means, which findings are deduplicated, who or what reviews them, and what proof is required before repair is accepted.
...
- You are changing or reviewing the Review Queue harness.
...
1. Name the exact boundary: `Review Queue` / `Cost efficiency`.
...
2. Write the observed failure mode in one sentence: "Scaled Review is needed because ..."
...
- Positive test: a normal request uses `Scaled Review` and produces the expected controlled behavior.
- Boundary test: an overreach, missing-input, unsafe, expensive, or unsupported request is blocked, downgraded, or escalated.
- Regression check: existing Review Queue behavior that should not change still works.
...
```markdown
## Scaled Review Harness Update

Boundary: Review Queue / Cost efficiency

...
description: "Use when working on the Compare harness and you need capability control: Adds comparison/voting strategies and configurable juror sets."
---
...
- Harness: Compare
- Axis: Capability
- Command: /voting-strategy

## What This Skill Does

Adds comparison/voting strategies and configurable juror sets.

Changing this harness changes model comparison: how many lanes run, which jurors participate, how voting works, and when early consensus is enough to stop spending.
...
- You are changing or reviewing the Compare harness.
...
1. Name the exact boundary: `Compare` / `Capability`.
...
- Comparison / voting strategy (skill)
- Juror set
...
2. Write the observed failure mode in one sentence: "Voting Strategy is needed because ..."
...
- Positive test: a normal request uses `Voting Strategy` and produces the expected controlled behavior.
- Boundary test: an overreach, missing-input, unsafe, expensive, or unsupported request is blocked, downgraded, or escalated.
- Regression check: existing Compare behavior that should not change still works.
...
```markdown
## Voting Strategy Harness Update

Boundary: Compare / Capability

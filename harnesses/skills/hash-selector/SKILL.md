---
name: hash-selector
description: "Use when working on the Payload harness and you need performance control: Picks a fast hash algorithm so fingerprinting adds minimal overhead."
---
...
- Harness: Payload
- Axis: Performance
- Command: /hash-selector

## What This Skill Does

Picks a fast hash algorithm so fingerprinting adds minimal overhead.
...
1. Name the exact boundary: `Payload` / `Performance`.
...
2. Write the observed failure mode in one sentence: "Hash Selector is needed because ..."
...
- Positive test: a normal request uses `Hash Selector` and produces the expected controlled behavior.
...
```markdown
## Hash Selector Harness Update

Boundary: Payload / Performance

---
name: retry-cap
description: "Use when working on the Fallback harness and you need cost efficiency control: Caps fallback tier so retries don't silently escalate to pricier models unless required."
---
...
1. Name the exact boundary: `Fallback` / `Cost efficiency`.
...
- Fallback-tier cap (don't escalate to pricier on retry unless needed)
...
2. Write the observed failure mode in one sentence: "Retry Cap is needed because ..."
...
- Positive test: a normal request uses `Retry Cap` and produces the expected controlled behavior.
...
```markdown
## Retry Cap Harness Update

Boundary: Fallback / Cost efficiency

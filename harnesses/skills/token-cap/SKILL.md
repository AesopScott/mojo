---
name: token-cap
description: "Use when working on the Route harness and you need cost efficiency control: Enforces token ceilings per route so a single dispatch cannot overspend."
---
...
Enforces token ceilings per route so a single dispatch cannot overspend.
...
1. Name the exact boundary: `Route` / `Cost efficiency`.
...
2. Write the observed failure mode in one sentence: "Token Cap is needed because ..."
...
- Positive test: a normal request uses `Token Cap` and produces the expected controlled behavior.
...
```markdown
## Token Cap Harness Update

Boundary: Route / Cost efficiency

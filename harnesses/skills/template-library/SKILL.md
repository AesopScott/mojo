...
description: "Use when working on the Prompt harness and you need capability control: Adds a template library and injection-order rules for new prompt shapes."
---
...
Adds a template library and injection-order rules for new prompt shapes.
...
1. Name the exact boundary: `Prompt` / `Capability`.
...
- Template library (plugin)
- Injection-order rules
...
2. Write the observed failure mode in one sentence: "Template Library is needed because ..."
...
- Positive test: a normal request uses `Template Library` and produces the expected controlled behavior.
...
```markdown
## Template Library Harness Update

Boundary: Prompt / Capability

---
name: cheap-pinner
description: "Use when working on the Identity harness and you need cost efficiency control: Pins to the cheapest sufficient model version when capability allows, lowering per-call cost."
---
...
Pins to the cheapest sufficient model version when capability allows, lowering per-call cost.
...
- Pin cheaper version when adequate
...
2. Write the observed failure mode in one sentence: "Cheap Pinner is needed because ..."
...
- Positive test: a normal request uses `Cheap Pinner` and produces the expected controlled behavior.

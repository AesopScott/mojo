---
name: tier-constraint
description: "Use when working on the Policy harness and you need cost efficiency control: Adds a constraint that forces the cheapest allowed tier when policy permits."
---
...
- Command: /tier-constraint

## What This Skill Does

Adds a constraint that forces the cheapest allowed tier when policy permits.

Changing this harness changes the rule set for the call: task constraints, provider constraints, tier limits, and route rules that decide what request shapes are permitted.
...
- You are changing or reviewing the Policy harness.
...
1. Name the exact boundary: `Policy` / `Cost efficiency`.
...
- Constraint that forces cheap tier when allowed
...
2. Write the observed failure mode in one sentence: "Tier Constraint is needed because ..."
...
- Positive test: a normal request uses `Tier Constraint` and produces the expected controlled behavior.
- Boundary test: an overreach, missing-input, unsafe, expensive, or unsupported request is blocked, downgraded, or escalated.
- Regression check: existing Policy behavior that should not change still works.
...
```markdown
## Tier Constraint Harness Update

Boundary: Policy / Cost efficiency

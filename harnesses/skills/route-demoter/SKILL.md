---
name: route-demoter
description: "Use when working on the Latency harness and you need performance control: Sets latency SLOs and demotes routes that breach them so fast paths win."
---
...
- Harness: Latency
- Axis: Performance
- Command: /route-demoter

## What This Skill Does

Sets latency SLOs and demotes routes that breach them so fast paths win.
...
1. Name the exact boundary: `Latency` / `Performance`.
...
2. Write the observed failure mode in one sentence: "Route Demoter is needed because ..."
...
- Positive test: a normal request uses `Route Demoter` and produces the expected controlled behavior.
...
```markdown
## Route Demoter Harness Update

Boundary: Latency / Performance

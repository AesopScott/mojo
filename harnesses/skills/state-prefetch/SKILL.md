---
name: state-prefetch
description: "Use when working on the Session Manager harness and you need performance control: Batches write-through and prefetches resume state so session open, resume, and worktree migration run with less stall."
---
...
- Axis: Performance
- Command: /state-prefetch

## What This Skill Does

Batches write-through and prefetches resume state so session open, resume, and worktree migration run with less stall.
...
1. Name the exact boundary: `Session Manager` / `Performance`.
...
2. Write the observed failure mode in one sentence: "State Prefetch is needed because ..."
...
- Positive test: a normal request uses `State Prefetch` and produces the expected controlled behavior.
...
```markdown
## State Prefetch Harness Update

Boundary: Session Manager / Performance

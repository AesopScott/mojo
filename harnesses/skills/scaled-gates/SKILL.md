---
name: scaled-gates
description: "Use when working on the Release Readiness harness and you need cost efficiency control: Scales gate-check scope to release size and reuses prior proof bundles to cut release-prep cost."
---
...
- Command: /scaled-gates

## What This Skill Does

Scales gate-check scope to release size and reuses prior proof bundles to cut release-prep cost.

Changing this harness changes deployment permission: which proof package is enough, which risks block release, which rollback path is required, and when the system can declare a build ready.
...
- You are changing or reviewing the Release Readiness harness.
...
1. Name the exact boundary: `Release Readiness` / `Cost efficiency`.
...
- Gate-check scope by release size
- Reuse prior proof bundles
...
2. Write the observed failure mode in one sentence: "Scaled Gates is needed because ..."
...
- Positive test: a normal request uses `Scaled Gates` and produces the expected controlled behavior.
- Boundary test: an overreach, missing-input, unsafe, expensive, or unsupported request is blocked, downgraded, or escalated.
- Regression check: existing Release Readiness behavior that should not change still works.
...
```markdown
## Scaled Gates Harness Update

Boundary: Release Readiness / Cost efficiency

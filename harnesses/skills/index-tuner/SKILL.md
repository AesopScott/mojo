---
name: index-tuner
description: "Use when working on the Memory harness and you need performance control: Tunes the recall index, decay-sweep interval, and write batching so memory reads and writes stay fast as it grows."
---
...
Tunes the recall index, decay-sweep interval, and write batching so memory reads and writes stay fast as it grows.
...
1. Name the exact boundary: `Memory` / `Performance`.
...
2. Write the observed failure mode in one sentence: "Index Tuner is needed because ..."
...
- Positive test: a normal request uses `Index Tuner` and produces the expected controlled behavior.
...
```markdown
## Index Tuner Harness Update

Boundary: Memory / Performance

---
name: budget-check
description: "Use when working on the Cost harness and you need performance control: Checks budget before dispatch so over-budget work is stopped before it spends."
---
...
Checks budget before dispatch so over-budget work is stopped before it spends.
...
1. Name the exact boundary: `Cost` / `Performance`.
...
- Budget pre-check before dispatch
...
2. Write the observed failure mode in one sentence: "Budget Check is needed because ..."
...
- Positive test: a normal request uses `Budget Check` and produces the expected controlled behavior.
...
```markdown
## Budget Check Harness Update

Boundary: Cost / Performance

---
name: juror-panel
description: "Use when working on the Compare harness and you need cost efficiency control: Caps lane count and uses cheaper juror models to bound comparison cost."
---
...
Caps lane count and uses cheaper juror models to bound comparison cost.
...
1. Name the exact boundary: `Compare` / `Cost efficiency`.
...
2. Write the observed failure mode in one sentence: "Juror Panel is needed because ..."
...
- Positive test: a normal request uses `Juror Panel` and produces the expected controlled behavior.
...
```markdown
## Juror Panel Harness Update

Boundary: Compare / Cost efficiency

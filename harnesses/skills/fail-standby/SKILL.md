---
name: fail-standby
description: "Use when working on the Fallback harness and you need performance control: Sets tight fail timeouts and keeps a pre-warmed standby so blocked routes recover quickly."
---
...
- Harness: Fallback
- Axis: Performance
- Command: /fail-standby

## What This Skill Does

Sets tight fail timeouts and keeps a pre-warmed standby so blocked routes recover quickly.

Changing this harness changes recovery behavior: when a model path is considered failed, which backup path is tried, how retries are capped, and whether degradation is acceptable.
...
- You are changing or reviewing the Fallback harness.
...
1. Name the exact boundary: `Fallback` / `Performance`.
...
2. Write the observed failure mode in one sentence: "Fail Standby is needed because ..."
...
- Positive test: a normal request uses `Fail Standby` and produces the expected controlled behavior.
- Boundary test: an overreach, missing-input, unsafe, expensive, or unsupported request is blocked, downgraded, or escalated.
- Regression check: existing Fallback behavior that should not change still works.
...
```markdown
## Fail Standby Harness Update

Boundary: Fallback / Performance

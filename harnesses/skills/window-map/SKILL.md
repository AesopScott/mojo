---
name: window-map
description: "Use when working on the Context harness and you need capability control: Adds packing strategies and a per-model window map so context fit adapts to each model."
---
...
Adds packing strategies and a per-model window map so context fit adapts to each model.
...
1. Name the exact boundary: `Context` / `Capability`.
...
- Packing strategy (skill)
- Per-model window map
...
2. Write the observed failure mode in one sentence: "Window Map is needed because ..."
...
- Positive test: a normal request uses `Window Map` and produces the expected controlled behavior.
...
```markdown
## Window Map Harness Update

Boundary: Context / Capability

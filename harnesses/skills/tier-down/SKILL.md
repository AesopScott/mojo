---
name: tier-down
description: "Use when working on the Model Router harness and you need cost efficiency control: Routes cheap tasks to lower tiers, enforces per-route token ceilings, and reuses cache hits before any paid dispatch."
---
...
1. Name the exact boundary: `Model Router` / `Cost efficiency`.
...
- Tier-down policy for cheap tasks
- Per-route token ceilings
- Cache-hit reuse before dispatch
...
2. Write the observed failure mode in one sentence: "Tier Down is needed because ..."
...
- Positive test: a normal request uses `Tier Down` and produces the expected controlled behavior.
...
```markdown
## Tier Down Harness Update

Boundary: Model Router / Cost efficiency

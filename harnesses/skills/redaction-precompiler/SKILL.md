---
name: redaction-precompiler
description: "Use when working on the Privacy harness and you need performance control: Precompiles redaction matchers so leakage scanning stays fast."
---
...
- Harness: Privacy
- Axis: Performance
- Command: /redaction-precompiler

## What This Skill Does

Precompiles redaction matchers so leakage scanning stays fast.

Changing this harness changes data exposure: which secrets, credentials, personal data, or sensitive classes are redacted before the model request leaves the system.
...
- You are changing or reviewing the Privacy harness.
...
1. Name the exact boundary: `Privacy` / `Performance`.
...
2. Write the observed failure mode in one sentence: "Redaction Precompiler is needed because ..."
...
- Positive test: a normal request uses `Redaction Precompiler` and produces the expected controlled behavior.
- Boundary test: an overreach, missing-input, unsafe, expensive, or unsupported request is blocked, downgraded, or escalated.
- Regression check: existing Privacy behavior that should not change still works.
...
```markdown
## Redaction Precompiler Harness Update

Boundary: Privacy / Performance

...
description: "Use when working on the Guardrails harness and you need cost efficiency control: Moves scans to high-value trigger points (pre-commit over continuous) and batches rules to reduce guardrail compute."
---
...
- Command: /trigger-optimizer

## What This Skill Does

Moves scans to high-value trigger points (pre-commit over continuous) and batches rules to reduce guardrail compute.

Changing this harness changes the permission boundary around the work: which actions are blocked, which risky moves need approval, what secrets are protected, and which credentials remain out of reach.
...
- You are changing or reviewing the Guardrails harness.
...
1. Name the exact boundary: `Guardrails` / `Cost efficiency`.
...
- Scan trigger points (pre-commit vs. continuous)
- Rule batching
...
2. Write the observed failure mode in one sentence: "Trigger Optimizer is needed because ..."
...
- Positive test: a normal request uses `Trigger Optimizer` and produces the expected controlled behavior.
- Boundary test: an overreach, missing-input, unsafe, expensive, or unsupported request is blocked, downgraded, or escalated.
- Regression check: existing Guardrails behavior that should not change still works.
...
```markdown
## Trigger Optimizer Harness Update

Boundary: Guardrails / Cost efficiency

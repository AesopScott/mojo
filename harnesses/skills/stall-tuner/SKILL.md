---
name: stall-tuner
description: "Use when working on the Health Monitor harness and you need performance control: Calibrates heartbeat interval, stall threshold, and anomaly window so genuine stalls surface fast without false alarms."
---
...
Calibrates heartbeat interval, stall threshold, and anomaly window so genuine stalls surface fast without false alarms.
...
1. Name the exact boundary: `Health Monitor` / `Performance`.
...
2. Write the observed failure mode in one sentence: "Stall Tuner is needed because ..."
...
- Positive test: a normal request uses `Stall Tuner` and produces the expected controlled behavior.
...
```markdown
## Stall Tuner Harness Update

Boundary: Health Monitor / Performance

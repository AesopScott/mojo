...
description: "Use when working on the Provider harness and you need cost efficiency control: Chooses batch over realtime endpoints and compresses payloads where latency allows to reduce vendor cost."
---
...
Chooses batch over realtime endpoints and compresses payloads where latency allows to reduce vendor cost.
...
1. Name the exact boundary: `Provider` / `Cost efficiency`.
...
- Endpoint tier (batch vs. realtime)
- Payload compression
...
2. Write the observed failure mode in one sentence: "Batch Endpoints is needed because ..."
...
- Positive test: a normal request uses `Batch Endpoints` and produces the expected controlled behavior.
...
```markdown
## Batch Endpoints Harness Update

Boundary: Provider / Cost efficiency

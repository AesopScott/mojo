---
name: context-pruner
description: "Use when working on the Goal harness and you need cost efficiency control: Drops context not tied to the active objective to save tokens."
---
...
- Command: /context-pruner

## What This Skill Does

Drops context not tied to the active objective to save tokens.

Changing this harness changes objective binding: which orchestrator goal the call is attached to, what context is pruned as irrelevant, and how the response is judged against intent.
...
- You are changing or reviewing the Goal harness.
...
1. Name the exact boundary: `Goal` / `Cost efficiency`.
...
- Drop off-goal context to save tokens
...
2. Write the observed failure mode in one sentence: "Context Pruner is needed because ..."
...
- Positive test: a normal request uses `Context Pruner` and produces the expected controlled behavior.
- Boundary test: an overreach, missing-input, unsafe, expensive, or unsupported request is blocked, downgraded, or escalated.
- Regression check: existing Goal behavior that should not change still works.
...
```markdown
## Context Pruner Harness Update

Boundary: Goal / Cost efficiency

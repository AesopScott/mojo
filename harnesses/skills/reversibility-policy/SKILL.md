...
description: "Use when working on the Tool Registry harness and you need capability control: Registers new tools, permission policy, and rollback/reversibility definitions for gated execution."
---
...
1. Name the exact boundary: `Tool Registry` / `Capability`.
...
- Permission policy
- Reversibility / rollback definitions
...
2. Write the observed failure mode in one sentence: "Reversibility Policy is needed because ..."
...
- Positive test: a normal request uses `Reversibility Policy` and produces the expected controlled behavior.
...
```markdown
## Reversibility Policy Harness Update

Boundary: Tool Registry / Capability

---
name: dry-run
description: "Use when working on the Tool Registry harness and you need cost efficiency control: Enforces tool-call ceilings, dry-runs costly actions first, and reuses sandboxes to avoid wasted spend."
---
...
1. Name the exact boundary: `Tool Registry` / `Cost efficiency`.
...
2. Write the observed failure mode in one sentence: "Dry Run is needed because ..."
...
- Positive test: a normal request uses `Dry Run` and produces the expected controlled behavior.
...
```markdown
## Dry Run Harness Update

Boundary: Tool Registry / Cost efficiency

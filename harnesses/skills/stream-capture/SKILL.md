---
name: stream-capture
description: "Use when working on the Response harness and you need performance control: Captures responses as a stream rather than blocking so downstream work starts sooner."
---
...
Captures responses as a stream rather than blocking so downstream work starts sooner.
...
1. Name the exact boundary: `Response` / `Performance`.
...
2. Write the observed failure mode in one sentence: "Stream Capture is needed because ..."
...
- Positive test: a normal request uses `Stream Capture` and produces the expected controlled behavior.

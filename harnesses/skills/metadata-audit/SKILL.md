---
name: metadata-audit
description: "Use when working on the Payload harness and you need cost efficiency control: Stores hash-only metadata instead of full payloads for cheap auditability."
---
...
Stores hash-only metadata instead of full payloads for cheap auditability.

Changing this harness changes prompt proof: how the exact request is hashed, signed, or stored so the system can prove what was actually sent.
...
- You are changing or reviewing the Payload harness.
...
1. Name the exact boundary: `Payload` / `Cost efficiency`.
...
- Hash-only metadata (cheap audit)
...
2. Write the observed failure mode in one sentence: "Metadata Audit is needed because ..."
...
- Positive test: a normal request uses `Metadata Audit` and produces the expected controlled behavior.
- Boundary test: an overreach, missing-input, unsafe, expensive, or unsupported request is blocked, downgraded, or escalated.
- Regression check: existing Payload behavior that should not change still works.
...
```markdown
## Metadata Audit Harness Update

Boundary: Payload / Cost efficiency

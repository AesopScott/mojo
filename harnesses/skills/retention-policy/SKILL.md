...
description: "Use when working on the Memory harness and you need cost efficiency control: Applies expiry, size caps, and dedup-on-write to keep durable memory bounded."
---
...
1. Name the exact boundary: `Memory` / `Cost efficiency`.
...
2. Write the observed failure mode in one sentence: "Retention Policy is needed because ..."
...
- Positive test: a normal request uses `Retention Policy` and produces the expected controlled behavior.
...
```markdown
## Retention Policy Harness Update

Boundary: Memory / Cost efficiency

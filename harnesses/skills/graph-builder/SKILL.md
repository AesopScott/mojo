---
name: graph-builder
description: "Use when working on the File Navigation harness and you need performance control: Builds the ownership graph incrementally, caps impact-walk depth, and caches paths so navigation stays fast on large trees."
---
...
- Harness: File Navigation
- Axis: Performance
- Command: /graph-builder

## What This Skill Does

Builds the ownership graph incrementally, caps impact-walk depth, and caches paths so navigation stays fast on large trees.

Changing this harness changes how the system finds the right files: ownership paths, impact depth, capability-to-code links, and proof paths that shape where edits and verification happen.
...
- You are changing or reviewing the File Navigation harness.
...
1. Name the exact boundary: `File Navigation` / `Performance`.
...
- Graph-build incrementality
- Impact-walk depth cap
- Path cache
...
2. Write the observed failure mode in one sentence: "Graph Builder is needed because ..."
...
- Positive test: a normal request uses `Graph Builder` and produces the expected controlled behavior.
- Boundary test: an overreach, missing-input, unsafe, expensive, or unsupported request is blocked, downgraded, or escalated.
- Regression check: existing File Navigation behavior that should not change still works.
...
```markdown
## Graph Builder Harness Update

Boundary: File Navigation / Performance

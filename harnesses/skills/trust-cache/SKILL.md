---
name: trust-cache
description: "Use when working on the Trust harness and you need performance control: Caches trust state so valid routes skip redundant lookups."
---
...
Caches trust state so valid routes skip redundant lookups.
...
1. Name the exact boundary: `Trust` / `Performance`.
...
2. Write the observed failure mode in one sentence: "Trust Cache is needed because ..."
...
- Positive test: a normal request uses `Trust Cache` and produces the expected controlled behavior.
...
```markdown
## Trust Cache Harness Update

Boundary: Trust / Performance

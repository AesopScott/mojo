---
name: finding-dedup
description: "Use when working on the Review Queue harness and you need performance control: Collapses duplicate findings and batches clusters across parallel reviewer lanes so the queue clears faster."
---
...
- Axis: Performance
- Command: /finding-dedup

## What This Skill Does

Collapses duplicate findings and batches clusters across parallel reviewer lanes so the queue clears faster.
...
1. Name the exact boundary: `Review Queue` / `Performance`.
...
2. Write the observed failure mode in one sentence: "Finding Dedup is needed because ..."
...
- Positive test: a normal request uses `Finding Dedup` and produces the expected controlled behavior.
...
```markdown
## Finding Dedup Harness Update

Boundary: Review Queue / Performance

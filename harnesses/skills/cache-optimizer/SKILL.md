...
description: "Use when working on the Prompt harness and you need cost efficiency control: Maximizes prompt-cache hits and strips redundant context to cut input tokens."
---
...
1. Name the exact boundary: `Prompt` / `Cost efficiency`.
...
2. Write the observed failure mode in one sentence: "Cache Optimizer is needed because ..."
...
- Positive test: a normal request uses `Cache Optimizer` and produces the expected controlled behavior.
...
```markdown
## Cache Optimizer Harness Update

Boundary: Prompt / Cost efficiency

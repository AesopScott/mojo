---
name: call-parallelizer
description: "Use when working on the Tools harness and you need performance control: Caches results and runs safe tool calls in parallel to reduce wait."
---
...
- Harness: Tools
- Axis: Performance
- Command: /call-parallelizer

## What This Skill Does

Caches results and runs safe tool calls in parallel to reduce wait.
...
- You are changing or reviewing the Tools harness.
...
1. Name the exact boundary: `Tools` / `Performance`.
...
2. Write the observed failure mode in one sentence: "Call Parallelizer is needed because ..."
...
- Positive test: a normal request uses `Call Parallelizer` and produces the expected controlled behavior.
- Boundary test: an overreach, missing-input, unsafe, expensive, or unsupported request is blocked, downgraded, or escalated.
- Regression check: existing Tools behavior that should not change still works.
...
```markdown
## Call Parallelizer Harness Update

Boundary: Tools / Performance

---
name: tier-selector
description: "Use when working on the Select harness and you need cost efficiency control: Defaults selection to the lowest tier that meets the task to avoid overpaying for capability."
---
...
- Command: /tier-selector

## What This Skill Does

Defaults selection to the lowest tier that meets the task to avoid overpaying for capability.
...
1. Name the exact boundary: `Select` / `Cost efficiency`.
...
- Default to lowest sufficient tier
...
2. Write the observed failure mode in one sentence: "Tier Selector is needed because ..."
...
- Positive test: a normal request uses `Tier Selector` and produces the expected controlled behavior.
...
```markdown
## Tier Selector Harness Update

Boundary: Select / Cost efficiency

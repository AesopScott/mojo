---
name: sync-optimizer
description: "Use when working on the Interface harness and you need cost efficiency control: Replaces redundant polling with change-driven push updates and tuned snapshot intervals to cut wasted repaint and state-sync work."
---
...
- Command: /sync-optimizer

## What This Skill Does

Replaces redundant polling with change-driven push updates and tuned snapshot intervals to cut wasted repaint and state-sync work.

Changing this harness changes what the operator can see and act on: which session states are visible, which controls are exposed, how fast changes surface, and how much authority is usable from the command center.
...
- You are changing or reviewing the Interface harness.
...
1. Name the exact boundary: `Interface` / `Cost efficiency`.
...
- Poll-vs-push state sync
- Snapshot interval
- Suppress redundant repaint
...
2. Write the observed failure mode in one sentence: "Sync Optimizer is needed because ..."
...
- Positive test: a normal request uses `Sync Optimizer` and produces the expected controlled behavior.
- Boundary test: an overreach, missing-input, unsafe, expensive, or unsupported request is blocked, downgraded, or escalated.
- Regression check: existing Interface behavior that should not change still works.
...
```markdown
## Sync Optimizer Harness Update

Boundary: Interface / Cost efficiency

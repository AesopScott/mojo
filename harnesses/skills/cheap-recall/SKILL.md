...
1. Name the exact boundary: `Knowledge Retrieval` / `Cost efficiency`.
...
2. Write the observed failure mode in one sentence: "Cheap Recall is needed because ..."
...
- Positive test: a normal request uses `Cheap Recall` and produces the expected controlled behavior.

---
name: ci-gate
description: "Use when working on the Git Control harness and you need cost efficiency control: Minimizes branch-lease scope and gates CI triggers (rebase-vs-merge policy) to avoid redundant pipeline cost."
---
...
Minimizes branch-lease scope and gates CI triggers (rebase-vs-merge policy) to avoid redundant pipeline cost.
...
1. Name the exact boundary: `Git Control` / `Cost efficiency`.
...
2. Write the observed failure mode in one sentence: "CI Gate is needed because ..."
...
- Positive test: a normal request uses `CI Gate` and produces the expected controlled behavior.
...
```markdown
## CI Gate Harness Update

Boundary: Git Control / Cost efficiency

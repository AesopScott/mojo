...
1. Name the exact boundary: `Trace` / `Performance`.
...
2. Write the observed failure mode in one sentence: "Async Writer is needed because ..."
...
- Positive test: a normal request uses `Async Writer` and produces the expected controlled behavior.

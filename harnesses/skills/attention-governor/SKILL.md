...
description: "Use when working on the Orchestrator harness and you need cost efficiency control: Caps concurrent high-cost reasoning sessions per project and reclaims idle slices, holding total attention spend under a set ceiling."
---
...
- Command: /attention-governor

## What This Skill Does

Caps concurrent high-cost reasoning sessions per project and reclaims idle slices, holding total attention spend under a set ceiling.
...
1. Name the exact boundary: `Orchestrator` / `Cost efficiency`.
...
- Attention budget per project
- Ceiling on concurrent high-cost reasoning sessions
- Idle-session reclaim timer
...
2. Write the observed failure mode in one sentence: "Attention Governor is needed because ..."
...
- Positive test: a normal request uses `Attention Governor` and produces the expected controlled behavior.
...
```markdown
## Attention Governor Harness Update

Boundary: Orchestrator / Cost efficiency

---
name: fast-pinner
description: "Use when working on the Identity harness and you need performance control: Pins requests to the fastest adequate model variant to trim response time."
---
...
1. Name the exact boundary: `Identity` / `Performance`.
...
2. Write the observed failure mode in one sentence: "Fast Pinner is needed because ..."
...
- Positive test: a normal request uses `Fast Pinner` and produces the expected controlled behavior.
...
```markdown
## Fast Pinner Harness Update

Boundary: Identity / Performance

...
description: "Use when working on the Memory harness and you need capability control: Adds a memory schema, conflict-resolution logic, and provenance depth so recall stays trustworthy under contradiction."
---
...
Adds a memory schema, conflict-resolution logic, and provenance depth so recall stays trustworthy under contradiction.

Changing this harness changes what the system carries forward: what gets written, recalled, expired, deduplicated, or treated as conflicting, and how provenance protects future decisions.
...
- You are changing or reviewing the Memory harness.
...
1. Name the exact boundary: `Memory` / `Capability`.
...
2. Write the observed failure mode in one sentence: "Conflict Resolution is needed because ..."
...
- Positive test: a normal request uses `Conflict Resolution` and produces the expected controlled behavior.
- Boundary test: an overreach, missing-input, unsafe, expensive, or unsupported request is blocked, downgraded, or escalated.
- Regression check: existing Memory behavior that should not change still works.
...
```markdown
## Conflict Resolution Harness Update

Boundary: Memory / Capability

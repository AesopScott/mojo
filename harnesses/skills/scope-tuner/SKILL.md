---
name: scope-tuner
description: "Use when working on the Privacy harness and you need cost efficiency control: Tunes scan scope to the data actually present, avoiding full-payload scanning cost."
---
...
Tunes scan scope to the data actually present, avoiding full-payload scanning cost.
...
1. Name the exact boundary: `Privacy` / `Cost efficiency`.
...
2. Write the observed failure mode in one sentence: "Scope Tuner is needed because ..."
...
- Positive test: a normal request uses `Scope Tuner` and produces the expected controlled behavior.
...
```markdown
## Scope Tuner Harness Update

Boundary: Privacy / Cost efficiency

---
name: sign-scheme
description: "Use when working on the Payload harness and you need capability control: Adds configurable hash/sign schemes for verifiable prompt fingerprints."
---
...
1. Name the exact boundary: `Payload` / `Capability`.
...
2. Write the observed failure mode in one sentence: "Sign Scheme is needed because ..."
...
- Positive test: a normal request uses `Sign Scheme` and produces the expected controlled behavior.
...
```markdown
## Sign Scheme Harness Update

Boundary: Payload / Capability

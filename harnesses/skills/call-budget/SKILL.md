---
name: call-budget
description: "Use when working on the Network Protocol harness and you need cost efficiency control: Prefers local execution, budgets remote calls, and uses degraded-mode fallbacks to limit cross-node cost."
---
...
Prefers local execution, budgets remote calls, and uses degraded-mode fallbacks to limit cross-node cost.
...
1. Name the exact boundary: `Network Protocol` / `Cost efficiency`.
...
2. Write the observed failure mode in one sentence: "Call Budget is needed because ..."
...
- Positive test: a normal request uses `Call Budget` and produces the expected controlled behavior.
...
```markdown
## Call Budget Harness Update

Boundary: Network Protocol / Cost efficiency

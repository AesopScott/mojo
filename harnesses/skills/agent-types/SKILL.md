---
name: agent-types
description: "Use when working on the Sub-agent Dispatch harness and you need capability control: Registers new agent types, pluggable planning strategies, and enforced result schemas."
---
...
Registers new agent types, pluggable planning strategies, and enforced result schemas.
...
1. Name the exact boundary: `Sub-agent Dispatch` / `Capability`.
...
2. Write the observed failure mode in one sentence: "Agent Types is needed because ..."
...
- Positive test: a normal request uses `Agent Types` and produces the expected controlled behavior.
...
```markdown
## Agent Types Harness Update

Boundary: Sub-agent Dispatch / Capability

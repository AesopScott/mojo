...
description: "Use when working on the Verify harness and you need performance control: Verifies a sampled subset rather than every call where risk allows, cutting verification latency."
---
...
- Harness: Verify
- Axis: Performance
- Command: /sampling-policy

## What This Skill Does

Verifies a sampled subset rather than every call where risk allows, cutting verification latency.

Changing this harness changes response trust: which checks confirm provider, route, response, and evidence before the output is allowed to influence downstream work.
...
- You are changing or reviewing the Verify harness.
...
1. Name the exact boundary: `Verify` / `Performance`.
...
2. Write the observed failure mode in one sentence: "Sampling Policy is needed because ..."
...
- Positive test: a normal request uses `Sampling Policy` and produces the expected controlled behavior.
- Boundary test: an overreach, missing-input, unsafe, expensive, or unsupported request is blocked, downgraded, or escalated.
- Regression check: existing Verify behavior that should not change still works.
...
```markdown
## Sampling Policy Harness Update

Boundary: Verify / Performance

---
name: tool-policy
description: "Use when working on the Tools harness and you need capability control: Configures the allowed-tool set and tool-call policy per route."
---
...
Configures the allowed-tool set and tool-call policy per route.
...
1. Name the exact boundary: `Tools` / `Capability`.
...
2. Write the observed failure mode in one sentence: "Tool Policy is needed because ..."
...
- Positive test: a normal request uses `Tool Policy` and produces the expected controlled behavior.
...
```markdown
## Tool Policy Harness Update

Boundary: Tools / Capability

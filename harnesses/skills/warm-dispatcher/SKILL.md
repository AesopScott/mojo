---
name: warm-dispatcher
description: "Use when working on the Route harness and you need performance control: Raises dispatch concurrency and keeps routes warm so approved work reaches the model faster."
---
...
- Harness: Route
- Axis: Performance
- Command: /warm-dispatcher

## What This Skill Does

Raises dispatch concurrency and keeps routes warm so approved work reaches the model faster.

Changing this harness changes dispatch: how the approved request reaches the selected model path, what token ceiling applies, and how much concurrency the route is allowed to use.
...
- You are changing or reviewing the Route harness.
...
1. Name the exact boundary: `Route` / `Performance`.
...
2. Write the observed failure mode in one sentence: "Warm Dispatcher is needed because ..."
...
- Positive test: a normal request uses `Warm Dispatcher` and produces the expected controlled behavior.
- Boundary test: an overreach, missing-input, unsafe, expensive, or unsupported request is blocked, downgraded, or escalated.
- Regression check: existing Route behavior that should not change still works.
...
```markdown
## Warm Dispatcher Harness Update

Boundary: Route / Performance

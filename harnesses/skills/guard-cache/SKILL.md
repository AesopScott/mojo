---
name: guard-cache
description: "Use when working on the Safety harness and you need performance control: Caches guard evaluations so repeated safe patterns clear quickly."
---
...
1. Name the exact boundary: `Safety` / `Performance`.
...
2. Write the observed failure mode in one sentence: "Guard Cache is needed because ..."
...
- Positive test: a normal request uses `Guard Cache` and produces the expected controlled behavior.
...
```markdown
## Guard Cache Harness Update

Boundary: Safety / Performance

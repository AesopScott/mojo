---
name: route-table
description: "Use when working on the Route harness and you need capability control: Adds a configurable route table and dispatch hooks for new model paths."
---
...
Adds a configurable route table and dispatch hooks for new model paths.
...
1. Name the exact boundary: `Route` / `Capability`.
...
2. Write the observed failure mode in one sentence: "Route Table is needed because ..."
...
- Positive test: a normal request uses `Route Table` and produces the expected controlled behavior.
...
```markdown
## Route Table Harness Update

Boundary: Route / Capability

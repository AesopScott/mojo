---
name: skip-reviewer
description: "Use when working on the Trust harness and you need cost efficiency control: Skips re-review while trust remains valid to avoid repeat review cost."
---
...
- Command: /skip-reviewer

## What This Skill Does

Skips re-review while trust remains valid to avoid repeat review cost.

Changing this harness changes route trust state: whether a provider/model path is direct, reviewed, blocked, expired, or allowed to keep running without repeated review.
...
- You are changing or reviewing the Trust harness.
...
1. Name the exact boundary: `Trust` / `Cost efficiency`.
...
- Skip re-review while trust valid
...
2. Write the observed failure mode in one sentence: "Skip Reviewer is needed because ..."
...
- Positive test: a normal request uses `Skip Reviewer` and produces the expected controlled behavior.
- Boundary test: an overreach, missing-input, unsafe, expensive, or unsupported request is blocked, downgraded, or escalated.
- Regression check: existing Trust behavior that should not change still works.
...
```markdown
## Skip Reviewer Harness Update

Boundary: Trust / Cost efficiency

---
name: rerank-tuner
description: "Use when working on the Knowledge Retrieval harness and you need performance control: Tunes embedding model, top-k, rerank depth, and chunk size so retrieval returns the right context with less latency."
---
...
- Axis: Performance
- Command: /rerank-tuner

## What This Skill Does

Tunes embedding model, top-k, rerank depth, and chunk size so retrieval returns the right context with less latency.
...
1. Name the exact boundary: `Knowledge Retrieval` / `Performance`.
...
2. Write the observed failure mode in one sentence: "Rerank Tuner is needed because ..."
...
- Positive test: a normal request uses `Rerank Tuner` and produces the expected controlled behavior.
...
```markdown
## Rerank Tuner Harness Update

Boundary: Knowledge Retrieval / Performance

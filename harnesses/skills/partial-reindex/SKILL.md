...
description: "Use when working on the File Navigation harness and you need cost efficiency control: Re-indexes only changed regions and applies partial graph updates to avoid full-rebuild cost."
---
...
- Command: /partial-reindex

## What This Skill Does

Re-indexes only changed regions and applies partial graph updates to avoid full-rebuild cost.
...
1. Name the exact boundary: `File Navigation` / `Cost efficiency`.
...
2. Write the observed failure mode in one sentence: "Partial Reindex is needed because ..."
...
- Positive test: a normal request uses `Partial Reindex` and produces the expected controlled behavior.
...
```markdown
## Partial Reindex Harness Update

Boundary: File Navigation / Cost efficiency

...
description: "Use when working on the Model Router harness and you need capability control: Adds new vendor/model adapters and configurable fallback-chain depth without changing the routing core."
---
...
Adds new vendor/model adapters and configurable fallback-chain depth without changing the routing core.
...
1. Name the exact boundary: `Model Router` / `Capability`.
...
- Provider adapter set (plugin)
- Fallback chain depth
- Route-learning feedback source
...
2. Write the observed failure mode in one sentence: "Provider Adapters is needed because ..."
...
- Positive test: a normal request uses `Provider Adapters` and produces the expected controlled behavior.
...
```markdown
## Provider Adapters Harness Update

Boundary: Model Router / Capability

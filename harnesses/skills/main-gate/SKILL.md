---
name: main-gate
description: "Use when working on the Git Control harness and you need capability control: Adds branch-lease rules, risk-scoring weights, and a scripted main-write gate."
---
...
Adds branch-lease rules, risk-scoring weights, and a scripted main-write gate.
...
1. Name the exact boundary: `Git Control` / `Capability`.
...
- Branch-lease rules (config)
- Risk-scoring weights
- Main-write gate (script)
...
2. Write the observed failure mode in one sentence: "Main Gate is needed because ..."
...
- Positive test: a normal request uses `Main Gate` and produces the expected controlled behavior.
...
```markdown
## Main Gate Harness Update

Boundary: Git Control / Capability

---
name: worker-ceiling
description: "Use when working on the Sub-agent Dispatch harness and you need cost efficiency control: Sets per-stage worker-model tier, caps total agent count, and early-exits empty stages to bound dispatch cost."
---
...
1. Name the exact boundary: `Sub-agent Dispatch` / `Cost efficiency`.
...
2. Write the observed failure mode in one sentence: "Worker Ceiling is needed because ..."
...
- Positive test: a normal request uses `Worker Ceiling` and produces the expected controlled behavior.
...
```markdown
## Worker Ceiling Harness Update

Boundary: Sub-agent Dispatch / Cost efficiency

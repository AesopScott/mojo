...
description: "Use when working on the Proof Authority harness and you need performance control: Runs evidence crosschecks across lanes and caches prior proofs so sufficiency scoring returns faster."
---
...
1. Name the exact boundary: `Proof Authority` / `Performance`.
...
- Evidence-scoring depth
- Parallel crosscheck lanes
- Proof cache
...
2. Write the observed failure mode in one sentence: "Crosscheck Lanes is needed because ..."
...
- Positive test: a normal request uses `Crosscheck Lanes` and produces the expected controlled behavior.
...
```markdown
## Crosscheck Lanes Harness Update

Boundary: Proof Authority / Performance

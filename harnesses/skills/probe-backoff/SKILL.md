---
name: probe-backoff
description: "Use when working on the Health Monitor harness and you need cost efficiency control: Backs off probe frequency on idle sessions and samples instead of per-tick polling to cut monitoring overhead."
---
...
- Command: /probe-backoff

## What This Skill Does

Backs off probe frequency on idle sessions and samples instead of per-tick polling to cut monitoring overhead.

Changing this harness changes how unattended sessions stay alive: how quickly stalls are detected, when recovery starts, what counts as unhealthy, and which workers can keep moving without manual inspection.
...
- You are changing or reviewing the Health Monitor harness.
...
1. Name the exact boundary: `Health Monitor` / `Cost efficiency`.
...
- Probe frequency vs. idle backoff
- Sampling instead of per-tick polling
...
2. Write the observed failure mode in one sentence: "Probe Backoff is needed because ..."
...
- Positive test: a normal request uses `Probe Backoff` and produces the expected controlled behavior.
- Boundary test: an overreach, missing-input, unsafe, expensive, or unsupported request is blocked, downgraded, or escalated.
- Regression check: existing Health Monitor behavior that should not change still works.
...
```markdown
## Probe Backoff Harness Update

Boundary: Health Monitor / Cost efficiency

---
name: selection-rubric
description: "Use when working on the Select harness and you need capability control: Adds an authored selection rubric and capability catalog so new task types map to the right model."
---
...
- Harness: Select
- Axis: Capability
- Command: /selection-rubric

## What This Skill Does

Adds an authored selection rubric and capability catalog so new task types map to the right model.
...
1. Name the exact boundary: `Select` / `Capability`.
...
2. Write the observed failure mode in one sentence: "Selection Rubric is needed because ..."
...
- Positive test: a normal request uses `Selection Rubric` and produces the expected controlled behavior.
...
```markdown
## Selection Rubric Harness Update

Boundary: Select / Capability

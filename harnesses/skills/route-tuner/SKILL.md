---
name: route-tuner
description: "Use when working on the Model Router harness and you need performance control: Continuously retunes latency-vs-quality route weights and keeps warm pools ready so dispatch picks the fastest adequate path."
---
...
Continuously retunes latency-vs-quality route weights and keeps warm pools ready so dispatch picks the fastest adequate path.

Changing this harness changes where work goes before a model call is made: which provider path is preferred, when fallback is used, how route quality is learned, and how cost, speed, and quality are traded.
...
- You are changing or reviewing the Model Router harness.
...
1. Name the exact boundary: `Model Router` / `Performance`.
...
- Route scoring weights (latency vs. quality)
- Warm-pool / keepalive
- Speculative routing
...
2. Write the observed failure mode in one sentence: "Route Tuner is needed because ..."
...
- Positive test: a normal request uses `Route Tuner` and produces the expected controlled behavior.
- Boundary test: an overreach, missing-input, unsafe, expensive, or unsupported request is blocked, downgraded, or escalated.
- Regression check: existing Model Router behavior that should not change still works.
...
```markdown
## Route Tuner Harness Update

Boundary: Model Router / Performance

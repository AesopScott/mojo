...
description: "Use when working on the Cost harness and you need cost efficiency control: Sets per-project and per-task budgets with alert and halt thresholds and fine-grained token accounting."
---
...
- You are changing or reviewing the Cost harness.
...
1. Name the exact boundary: `Cost` / `Cost efficiency`.
...
- Per-project / per-task budgets
- Alert / halt thresholds
- Token-accounting granularity
...
2. Write the observed failure mode in one sentence: "Threshold Governor is needed because ..."
...
- Positive test: a normal request uses `Threshold Governor` and produces the expected controlled behavior.
- Boundary test: an overreach, missing-input, unsafe, expensive, or unsupported request is blocked, downgraded, or escalated.
- Regression check: existing Cost behavior that should not change still works.
...
```markdown
## Threshold Governor Harness Update

Boundary: Cost / Cost efficiency

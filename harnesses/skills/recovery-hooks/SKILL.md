---
name: recovery-hooks
description: "Use when working on the Health Monitor harness and you need capability control: Wires custom health checks and recovery triggers so a detected stall can route to a defined repair action."
---
...
1. Name the exact boundary: `Health Monitor` / `Capability`.
...
- Custom health checks (script)
- Alert routing hooks
- Auto-recovery trigger wiring
...
2. Write the observed failure mode in one sentence: "Recovery Hooks is needed because ..."
...
- Positive test: a normal request uses `Recovery Hooks` and produces the expected controlled behavior.
...
```markdown
## Recovery Hooks Harness Update

Boundary: Health Monitor / Capability

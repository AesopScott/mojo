...
1. Name the exact boundary: `Latency` / `Cost efficiency`.
...
2. Write the observed failure mode in one sentence: "Batch Mode is needed because ..."
...
- Positive test: a normal request uses `Batch Mode` and produces the expected controlled behavior.

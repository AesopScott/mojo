...
description: "Use when working on the Verify harness and you need capability control: Adds scripted checks and an evidence schema so new output types can be confirmed."
---
...
Adds scripted checks and an evidence schema so new output types can be confirmed.
...
1. Name the exact boundary: `Verify` / `Capability`.
...
2. Write the observed failure mode in one sentence: "Verification Checks is needed because ..."
...
- Positive test: a normal request uses `Verification Checks` and produces the expected controlled behavior.
...
```markdown
## Verification Checks Harness Update

Boundary: Verify / Capability

---
name: summarize-fit
description: "Use when working on the Context harness and you need cost efficiency control: Trims and summarizes context and reuses chunks to lower token spend."
---
...
1. Name the exact boundary: `Context` / `Cost efficiency`.
...
- Trim / summarize to reduce tokens
- Chunk reuse
...
2. Write the observed failure mode in one sentence: "Summarize Fit is needed because ..."
...
- Positive test: a normal request uses `Summarize Fit` and produces the expected controlled behavior.
...
```markdown
## Summarize Fit Harness Update

Boundary: Context / Cost efficiency

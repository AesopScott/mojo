---
name: process-hooks
description: "Use when working on the Response harness and you need capability control: Adds an artifact schema and post-process hooks for response handling."
---
...
- Harness: Response
- Axis: Capability
- Command: /process-hooks

## What This Skill Does

Adds an artifact schema and post-process hooks for response handling.

Changing this harness changes output capture: how returned content is streamed, stored, compressed, post-processed, and made available as a controlled artifact.
...
- You are changing or reviewing the Response harness.
...
1. Name the exact boundary: `Response` / `Capability`.
...
2. Write the observed failure mode in one sentence: "Process Hooks is needed because ..."
...
- Positive test: a normal request uses `Process Hooks` and produces the expected controlled behavior.
- Boundary test: an overreach, missing-input, unsafe, expensive, or unsupported request is blocked, downgraded, or escalated.
- Regression check: existing Response behavior that should not change still works.
...
```markdown
## Process Hooks Harness Update

Boundary: Response / Capability

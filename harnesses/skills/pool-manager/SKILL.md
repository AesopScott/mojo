---
name: pool-manager
description: "Use when working on the Provider harness and you need performance control: Pools connections, keeps them alive, and selects the nearest region to cut per-call connection overhead."
---
...
1. Name the exact boundary: `Provider` / `Performance`.
...
2. Write the observed failure mode in one sentence: "Pool Manager is needed because ..."
...
- Positive test: a normal request uses `Pool Manager` and produces the expected controlled behavior.
...
```markdown
## Pool Manager Harness Update

Boundary: Provider / Performance

---
name: scoped-guards
description: "Use when working on the Safety harness and you need cost efficiency control: Scopes guard checks by risk so low-risk calls run fewer checks."
---
...
- Command: /scoped-guards

## What This Skill Does

Scopes guard checks by risk so low-risk calls run fewer checks.

Changing this harness changes safety enforcement: which content or actions are blocked, which guards run for the risk level, and when the request cannot proceed.
...
- You are changing or reviewing the Safety harness.
...
1. Name the exact boundary: `Safety` / `Cost efficiency`.
...
2. Write the observed failure mode in one sentence: "Scoped Guards is needed because ..."
...
- Positive test: a normal request uses `Scoped Guards` and produces the expected controlled behavior.
- Boundary test: an overreach, missing-input, unsafe, expensive, or unsupported request is blocked, downgraded, or escalated.
- Regression check: existing Safety behavior that should not change still works.
...
```markdown
## Scoped Guards Harness Update

Boundary: Safety / Cost efficiency

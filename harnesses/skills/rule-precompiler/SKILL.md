---
name: rule-precompiler
description: "Use when working on the Policy harness and you need performance control: Precompiles and caches constraint rules so policy evaluation adds little overhead."
---
...
- Harness: Policy
- Axis: Performance
- Command: /rule-precompiler

## What This Skill Does

Precompiles and caches constraint rules so policy evaluation adds little overhead.
...
1. Name the exact boundary: `Policy` / `Performance`.
...
2. Write the observed failure mode in one sentence: "Rule Precompiler is needed because ..."
...
- Positive test: a normal request uses `Rule Precompiler` and produces the expected controlled behavior.
...
```markdown
## Rule Precompiler Harness Update

Boundary: Policy / Performance

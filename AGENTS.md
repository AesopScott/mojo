...
- **Deployment path:** Mojo deploys through GitHub/GitHub Actions to Cloudflare Pages/Workers using `wrangler.jsonc`, `.github/workflows/deploy.yml`, and `cloudflare/pages-worker.js`. Treat Cloudflare Pages/Workers as the production deployment source of truth. Do not use or recommend the old MochaHost/FTP deployment path; it is retired. Route commits, pushes, PRs, and production deployment actions through Release Management / Reid unless Scott explicitly approves a direct scoped deploy.

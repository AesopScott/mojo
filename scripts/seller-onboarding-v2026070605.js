...
<ol>
  ...
  <li><strong>Listing Fee:</strong> A $100 non-refundable listing fee applies per product submission unless Mojo waives it. The listing fee must be paid to Mojo by PayPal or Zelle before this agreement can be signed.</li>
...
<p>Last updated: July 2, 2026</p>`;
  ...
  let onboardingState = {};
  let listingFeeHandlersReady = false;
  ...
  function setupListingFeeHandlers() {
    if (listingFeeHandlersReady) return;

    const listingFeeForm = getRequiredElement('listing-fee-form');
    const submitListingFeeBtn = getRequiredElement('btn-submit-listing-fee');

    listingFeeForm.addEventListener('submit', async function (e) {
      e.preventDefault();

      try {
        showError('');
        submitListingFeeBtn.disabled = true;
        submitListingFeeBtn.textContent = 'Submitting...';

        const params = new URLSearchParams(window.location.search);
        const sellerToken = params.get('token');
        const email = params.get('email');
        const paymentMethod = getRequiredElement('listing-fee-method').value;
        const payerName = getRequiredElement('listing-fee-payer').value.trim();
        const paymentReference = getRequiredElement('listing-fee-reference').value.trim();

        if (!email || !sellerToken) {
          throw new Error('Missing onboarding link details. Please check your email for the correct link.');
        }
        if (!paymentMethod || !payerName || !paymentReference) {
          throw new Error('Please choose PayPal or Zelle and enter the payment confirmation details.');
        }

        const response = await fetch(
          `https://${CLOUD_FUNCTION_REGION}-${CLOUD_FUNCTION_PROJECT}.cloudfunctions.net/submitListingFeePayment`,
          {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              email,
              sellerToken,
              productId: onboardingState.productId || null,
              paymentMethod,
              payerName,
              paymentReference,
            }),
        ...
        submitListingFeeBtn.textContent = 'Submit Payment Details';
      }
    });

    listingFeeHandlersReady = true;
  }

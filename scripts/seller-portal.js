...
(function () {
  ...
  function listingFeePaymentHtml(product) {
    ...
    if (!product.listingFeePolarPriceId) {
      return '<div class="sync-note" style="margin-top:12px;">Mojo is preparing the listing fee checkout. Refresh this page in a moment.</div>';
    }

    return `
      <button class="button full" type="button" data-polar-checkout="${escapeAttribute(product.listingFeePolarPriceId)}" style="margin-top:12px;">
        Pay $100 Listing Fee
      </button>
    `;
  }

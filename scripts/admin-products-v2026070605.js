...
<div class="form-group">
      ...
      <div class="action-buttons">
        <button class="btn-approve" data-product-id="${productId}" ${sellerInviteSent ? 'disabled' : ''}>${sellerInviteSent ? 'Seller Contract Sent' : 'Approve and Send Seller Contract'}</button>
    ...
    approveBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        approveBtn.textContent = 'Approve and Send Seller Contract';
      }
    });
    ...
    const listingFeeLabel = listingFeeStatus === 'paid'
      ...
      : listingFeeStatus === 'waived'
        ...
        : listingFeeStatus === 'payment_submitted'
          ? 'Payment Submitted'
          : 'Pending';
    const listingFeePayment = product.listingFeePayment || {};
    const listingFeePaymentHtml = listingFeeStatus === 'payment_submitted'
      ? `<div class="seller-payout" style="border-color:#f59e0b;background:#fffbeb;">
          <h4 class="seller-payout-title">$100 Listing Fee Payment Submitted</h4>
          <div class="seller-payout-grid">
            <div class="meta-item">
              <div class="meta-label">Method</div>
              <div class="meta-value">${escapeHtml(formatPayoutMethod(listingFeePayment.method))}</div>
            </div>
            <div class="meta-item">
              <div class="meta-label">Paid From</div>
              <div class="meta-value">${escapeHtml(formatValue(listingFeePayment.payerName, 'Not provided'))}</div>
            </div>
            <div class="meta-item">

  ...
  const body = `
      <section class="aac-hero">
        ...
        <div class="aac-hero-media">
          ...
          <div class="aac-zoom-status" id="aac-zoom-status" hidden>
            <div class="aac-zoom-status-head">
              <span>Zoom activity</span>
              <b data-zoom-state>Waiting for events</b>
            </div>
            <div class="aac-zoom-metrics">
              <span><b data-zoom-created>0</b> meetings created</span>
              <span><b data-zoom-attendees>0</b> attendee count</span>
              <span><b data-zoom-updated>0</b> meetings updated</span>
            </div>
            <div class="aac-zoom-feed" data-zoom-feed></div>
          </div>
        </div>
      </section>
    </header>
  ...
  return pageShell({
    ...
    schema: [
      ...
      (function() {
        ...
        var state = panel.querySelector('[data-zoom-state]');
        var created = panel.querySelector('[data-zoom-created]');
        var attendees = panel.querySelector('[data-zoom-attendees]');
        var updated = panel.querySelector('[data-zoom-updated]');
        var feed = panel.querySelector('[data-zoom-feed]');
        ...
        function renderLatest(items) {
          ...
          if (!items || !items.length) {
            ...
            empty.textContent = 'No Zoom events received yet.';
            feed.appendChild(empty);
            return;
          }
          items.slice(0, 3).forEach(function(item) {
            var row = document.createElement('span');
            var age = relativeTime(item.receivedAt);
            row.textContent = item.label + (age ? ' - ' + age : '');
        ...
        function loadZoomStatus() {
          fetch('/api/zoom-status')
            ...
            .then(function(data) {
              ...
              setText(created, Number(data.counts.meeting_created || 0).toLocaleString());
              setText(attendees, Number(data.counts.participant_joined || 0).toLocaleString());
              setText(updated, Number(data.counts.meeting_updated || 0).toLocaleString());
              setText(state, data.latest && data.latest.length ? 'Live webhook feed' : 'Waiting for Zoom events');
        ...
        loadZoomStatus();
        window.setInterval(loadZoomStatus, 60000);
      })();
    </script>
  </body>`);
}
